...
years = np.arange(1981,1980,-1) # fill in the years backward
yearpart = np.arange(365,-1,-1) # for a full (leap)year fill in (365,-1,-1)
boundary = 6 # with 8 the vertical separation is at 812.83 hPa for surface pressure = 1031.25 hPa, which corresponds to k=47 (ERA-Interim)
...
def data_path(previous_data_to_load,yearnumber,a):
    ...
    load_fluxes_and_storages = os.path.join(fluxes_and_states_dir, str(yearnumber) + '-' + str(a) + 'fluxes_storages.mat')
...
def get_Sa_track_backward(latitude,longitude,count_time,divt,Kvf,Region,Fa_E_top,Fa_N_top,Fa_E_down,Fa_N_down,
                                       ...
                                       #shadow_storage_top,shadow_storage_down):
...
for i in range(len(years)):
    ...
    fluxes_and_states_dir = get_FS_dir_wildNumStr(yearnumber)
